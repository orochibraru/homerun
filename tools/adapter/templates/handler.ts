...
export const getHandler = () => {
	// Homerun delta from upstream : upstream calls `server.websocket()`
	// unconditionally here. That accessor only exists when @sveltejs/kit's
	// generated server code happens to match upstream's own regex-graft
	// (dropped entirely from this vendored copy, see index.ts's header
	// comment), which it doesn't against our pinned @sveltejs/kit@^2.70.3,
	// verified. This app has no websocket hook (see CLAUDE.md's Web terminal
	// section), so a missing accessor just means "no websocket support",
