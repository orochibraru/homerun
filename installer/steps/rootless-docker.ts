 ...
 */
export async function installDockerEngine(run: StepRunner): Promise<void> {
...
export async function installRootlessPrereqs(
	run: StepRunner,
	pm: PackageManager,
): Promise<void> {
	const pkgs =
...
export async function ensureRootlessUser(
	run: StepRunner,
	username: string,
): Promise<void> {
	...
	await run.run(["useradd", "--create-home", "--shell", "/bin/bash", username]);
}
 ...
 */
export async function installRootlessDocker(
	run: StepRunner,
	username: string,
): Promise<string> {
