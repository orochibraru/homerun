...
from the root `openapi.json`, itself built from source by
`scripts/generate-openapi.ts` (the root `bun run gen`, which also regenerates
`homerun.schema.json` and runs as part of `build:app`). All three are checked
in; regenerate after any REST API route change, CI's "Codegen is current" step
fails on a stale snapshot since `openapi-fetch` itself can't detect one at
compile time). Auth is `x-api-key`/`--api-key`, same header the REST API's own
hooks check first for a non-cookie caller. Commands:
