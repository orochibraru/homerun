...
- **Custom triggers.** Wrap controls in `Dialog.Trigger` /
  `AlertDialog.Trigger`, or control open state with `bind:open` on the root, see
  component docs.
...
- [rules/forms.md](./rules/forms.md), Field.FieldGroup, Field.Field, InputGroup,
  ToggleGroup, Field.FieldSet, validation states
