...
<div class="mx-auto px-6 py-10 sm:py-12">
    ...
    <details
        class="mb-8 rounded-xl border border-(--border) bg-(--bg-card) lg:hidden"
    >
        <summary
        ...
        <nav class="flex flex-col gap-0.5 border-t border-(--border) p-2">
            {#each docPages as doc (doc.slug)}
                <a
                    href="/docs/{doc.slug}"
                    class={navLinkClass(`/docs/${doc.slug}`)}>{doc.title}</a
                >
            {/each}
            <a href="/docs/api" class={navLinkClass("/docs/api")}
                >API reference</a
            >
        </nav>
    </details>
    ...
    <div class="flex items-start gap-10">
        ...
        <aside class="sticky top-24 hidden w-56 shrink-0 lg:block">
            <p
                class="mb-3 px-3 text-xs font-semibold tracking-wide text-(--text-subtle) uppercase"
            >
                Guides
            </p>
            <nav class="flex flex-col gap-0.5 text-sm">
                {#each docPages as doc (doc.slug)}
                    <a
                        href="/docs/{doc.slug}"
                        class="relative rounded-lg py-1.5 pr-3 pl-3 transition-colors {isActive(
                            `/docs/${doc.slug}`,
                        )
                            ? 'bg-(--accent-soft) font-medium text-(--accent-strong)'
                            : 'text-(--text-muted) hover:bg-(--bg-card) hover:text-(--text)'}"
            ...
            <a
                href="/docs/api"
                class="block rounded-lg px-3 py-1.5 text-sm transition-colors {isActive(
                    '/docs/api',
                )
                    ? 'bg-(--accent-soft) font-medium text-(--accent-strong)'
                    : 'text-(--text-muted) hover:bg-(--bg-card) hover:text-(--text)'}"
