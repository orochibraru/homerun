...
func TestReleaseAssetURL(t *testing.T) {
	latest := ReleaseAssetURL("latest", "homerun-agent-amd64.gz")
	...
	pinned := ReleaseAssetURL("v1.2.3", "homerun-cli-arm64.gz")
...
func TestImageRef(t *testing.T) {
	if got := ImageRef("latest"); got != "docker.io/orochibraru/homerun:latest" {
		t.Errorf("got %q", got)
	}
	if got := ImageRef("v1.2.3"); got != "docker.io/orochibraru/homerun:v1.2.3" {
		t.Errorf("got %q", got)
	}
}
