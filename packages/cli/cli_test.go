...
func runCLI(t *testing.T, fn func()) (string, string) {
	t.Helper()
	original := fail
	fail = func(message string) { panic(cliExit{message}) }
...
func fakeAPI(t *testing.T, handler http.HandlerFunc) (*Client, *[]seenRequest) {
	t.Helper()
	seen := &[]seenRequest{}
	...
	return newClient(ClientConfig{APIKey: "k", BaseURL: server.URL}), seen
}

// jsonAPI is a fake instance that answers every request with the same JSON body.
func jsonAPI(t *testing.T, body string) (*Client, *[]seenRequest) {
	t.Helper()
...
func TestServicesListTable(t *testing.T) {
	...
	out, failed := runCLI(t, func() { servicesList(client, ListArgs{Page: 2, Search: "web"}) })
	if failed != "" {
...
func TestServicesListJSONPrintsTheBodyVerbatim(t *testing.T) {
	...
	out, failed := runCLI(t, func() { servicesList(client, ListArgs{JSON: true}) })
	if failed != "" {
...
func TestServicesListRejectsAnUnexpectedShape(t *testing.T) {
	...
	if _, failed := runCLI(t, func() { servicesList(client, ListArgs{}) }); failed == "" {
...
func TestServiceGet(t *testing.T) {
	...
	out, failed := runCLI(t, func() { serviceGet(client, "svc 1") })
	if failed != "" {
		t.Fatalf("failed with %q", failed)
	}
...
func TestServiceActionHitsThePerActionEndpoint(t *testing.T) {
	for _, action := range []string{"deploy", "start", "stop", "restart"} {
		t.Run(action, func(t *testing.T) {
			...
			out, failed := runCLI(t, func() { serviceAction(client, action, "svc-1") })
			if failed != "" {
				t.Fatalf("failed with %q", failed)
			}
...
func TestServiceDelete(t *testing.T) {
	for name, force := range map[string]bool{"plain": false, "forced": true} {
		t.Run(name, func(t *testing.T) {
			...
			out, failed := runCLI(t, func() { serviceDelete(client, "svc-1", force) })
			if failed != "" {
				t.Fatalf("failed with %q", failed)
			}
...
func TestServiceDeleteSurfacesTheAPIError(t *testing.T) {
	...
	_, failed := runCLI(t, func() { serviceDelete(client, "svc-1", false) })
...
func TestServiceWebhook(t *testing.T) {
	...
	out, failed := runCLI(t, func() { serviceWebhook(client, "svc-1") })
	if failed != "" {
		t.Fatalf("failed with %q", failed)
	}
...
func TestRevisionsList(t *testing.T) {
	...
	out, failed := runCLI(t, func() { revisionsList(client, "svc-1", false) })
	if failed != "" {
		t.Fatalf("failed with %q", failed)
	}
	...
	out, failed = runCLI(t, func() { revisionsList(client, "svc-1", true) })
	if failed != "" {
		t.Fatalf("failed with %q", failed)
	}
...
func TestServiceLogsStreamsToStdout(t *testing.T) {
	...
	out, failed := runCLI(t, func() { serviceLogs(client, "svc-1", true, 50) })
	if failed != "" {
		t.Fatalf("failed with %q", failed)
	}
	if out != "line one\nline two\n" {
...
func TestServiceLogsWithoutOptionsSendsNoQuery(t *testing.T) {
	...
	if _, failed := runCLI(t, func() { serviceLogs(client, "svc-1", false, 0) }); failed != "" {
		t.Fatalf("failed with %q", failed)
	}
...
func TestServiceLogsReportsTheAPIError(t *testing.T) {
	...
	_, failed := runCLI(t, func() { serviceLogs(client, "svc-1", false, 0) })
...
func TestServiceLogsReportsATransportFailure(t *testing.T) {
	client := newClient(ClientConfig{APIKey: "k", BaseURL: deadServerURL(t)})
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			out, failed := runCLI(t, func() {
				serviceRollback(client, "svc-1", test.revisionID, test.restoreConfig)
			})
			if failed != "" {
				t.Fatalf("failed with %q", failed)
			}
...
func TestScansList(t *testing.T) {
	...
	out, failed := runCLI(t, func() { scansList(client, "svc-1", ListArgs{PerPage: 1}) })
	if failed != "" {
		t.Fatalf("failed with %q", failed)
	}
	...
	out, failed = runCLI(t, func() { scansList(client, "svc-1", ListArgs{JSON: true}) })
	if failed != "" {
		t.Fatalf("failed with %q", failed)
	}
...
func TestScanGet(t *testing.T) {
	for name, scanID := range map[string]string{"latest": "latest", "by id": "scan-9"} {
		t.Run(name, func(t *testing.T) {
			...
			var scan ImageScan
			out, failed := runCLI(t, func() { scan = scanGet(client, "svc-1", scanID, false) })
			if failed != "" {
				t.Fatalf("failed with %q", failed)
			}
...
func TestScanGetJSONSkipsTheSummary(t *testing.T) {
	...
	out, failed := runCLI(t, func() { scanGet(client, "svc-1", "latest", true) })
	if failed != "" {
		t.Fatalf("failed with %q", failed)
	}
...
func TestPrintScanNotesTruncationAndErrors(t *testing.T) {
	scan := ImageScan{
		Counts:        SeverityCounts{Critical: 1, Unknown: 2},
		Digest:        "sha256:abc",
		Error:         "trivy died",
		ID:            "scan-1",
		ImageRef:      "nginx:1.27",
		...
		Status:        "failed",
		TotalFindings: 40,
		Findings:      []ScanFinding{{ID: "CVE-1", Pkg: "zlib", Severity: "critical", Title: "boom"}},
	}

	out, _ := runCLI(t, func() { printScan(scan) })
...
func TestQueueScan(t *testing.T) {
	t.Run("queued", func(t *testing.T) {
		...
		jobID := ""
		if _, failed := runCLI(t, func() { jobID = queueScan(client, "svc-1", false) }); failed != "" {
			t.Fatalf("failed with %q", failed)
		}
		if jobID != "job-1" {
	...
	t.Run("409 is reused when waiting", func(t *testing.T) {
		...
		jobID := ""
		if _, failed := runCLI(t, func() { jobID = queueScan(client, "svc-1", true) }); failed != "" {
	...
	t.Run("409 fails without wait", func(t *testing.T) {
		...
		if _, failed := runCLI(t, func() { queueScan(client, "svc-1", false) }); !strings.Contains(failed, "409") {
	...
	t.Run("transport failure", func(t *testing.T) {
		client := newClient(ClientConfig{APIKey: "k", BaseURL: deadServerURL(t)})
		if _, failed := runCLI(t, func() { queueScan(client, "svc-1", true) }); failed == "" {
			t.Error("an unreachable instance should fail")
		}
	})
}
...
func TestWaitForJobPollsUntilTerminal(t *testing.T) {
	...
	if _, failed := runCLI(t, func() {
		status, jobError = waitForJob(client, "job-1", time.Millisecond, time.Minute)
	}); failed != "" {
		t.Fatalf("failed with %q", failed)
	}
...
func TestWaitForJobGivesUp(t *testing.T) {
	...
	_, failed := runCLI(t, func() { waitForJob(client, "job-1", time.Millisecond, -time.Second) })
...
func scanAPI(t *testing.T, jobStatus, scanBody string) (*Client, *[]seenRequest) {
	t.Helper()
...
func TestServiceScanWithoutWaitPrintsTheJobID(t *testing.T) {
	...
	out, failed := runCLI(t, func() { serviceScan(client, "svc-1", ScanArgs{}) })
	if failed != "" {
		t.Fatalf("failed with %q", failed)
	}
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			_, failed := runCLI(t, func() {
				serviceScan(client, "svc-1", ScanArgs{FailOn: test.failOn, Wait: true})
			})
...
func TestServiceScanReportsAFailedJob(t *testing.T) {
	...
	_, failed := runCLI(t, func() { serviceScan(client, "svc-1", ScanArgs{Wait: true}) })
...
func TestInstanceStatus(t *testing.T) {
	...
	out, failed := runCLI(t, func() { instanceStatus(client, false) })
	if failed != "" {
		t.Fatalf("failed with %q", failed)
	}
	...
	out, failed = runCLI(t, func() { instanceStatus(client, true) })
	if failed != "" {
		t.Fatalf("failed with %q", failed)
	}
...
func TestInstanceUpdateWithoutWaitReturnsImmediately(t *testing.T) {
	...
	out, failed := runCLI(t, func() { instanceUpdate(client, false, 0) })
	if failed != "" {
		t.Fatalf("failed with %q", failed)
	}
...
func TestInstanceUpdateGivesUp(t *testing.T) {
	...
	_, failed := runCLI(t, func() { instanceUpdate(client, true, time.Nanosecond) })
...
func TestCurrentVersionOrEmptyToleratesAnUnhealthyInstance(t *testing.T) {
	t.Run("unreachable", func(t *testing.T) {
		client := newClient(ClientConfig{APIKey: "k", BaseURL: deadServerURL(t)})
		if got := currentVersionOrEmpty(client); got != "" {
	...
	t.Run("healthy", func(t *testing.T) {
		...
		if got := currentVersionOrEmpty(client); got != "1.2.0" {
...
func TestStacksList(t *testing.T) {
	...
	out, failed := runCLI(t, func() { stacksList(client, ListArgs{Page: 2}) })
	if failed != "" {
		t.Fatalf("failed with %q", failed)
	}
	...
	out, failed = runCLI(t, func() { stacksList(client, ListArgs{JSON: true}) })
...
func TestTemplatesList(t *testing.T) {
	...
	out, failed := runCLI(t, func() { templatesList(client, ListArgs{}) })
	if failed != "" {
		t.Fatalf("failed with %q", failed)
	}
	...
	out, failed = runCLI(t, func() { templatesList(client, ListArgs{JSON: true}) })
...
func TestPollForApprovalStoresTheKey(t *testing.T) {
	...
	out, failed := runCLI(t, func() {
		pollForApproval(server.URL, deviceStart{DeviceCode: "dc-1", ExpiresIn: 60, Interval: 1})
	})
	if failed != "" {
		t.Fatalf("failed with %q", failed)
	}
...
func TestPollForApprovalFailures(t *testing.T) {
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			_, failed := runCLI(t, func() {
				pollForApproval(server.URL, deviceStart{DeviceCode: "dc-1", ExpiresIn: test.expiresIn})
			})
			if !strings.Contains(failed, test.want) {
				t.Errorf("want %q, got %q", test.want, failed)
			}
			if readStoredConfig() != nil {
...
func TestPollForApprovalKeepsPollingWhilePending(t *testing.T) {
	...
	if _, failed := runCLI(t, func() {
		pollForApproval(server.URL, deviceStart{DeviceCode: "dc-1", ExpiresIn: 60})
	}); failed != "" {
		t.Fatalf("failed with %q", failed)
	}
	if polls != 3 {
...
func TestPollForApprovalReportsATransportFailure(t *testing.T) {
	...
	_, failed := runCLI(t, func() {
		pollForApproval(url, deviceStart{DeviceCode: "dc-1", ExpiresIn: 60})
	})
...
func TestStartDeviceAuth(t *testing.T) {
	t.Run("ok", func(t *testing.T) {
		...
		if _, failed := runCLI(t, func() { start = startDeviceAuth(server.URL) }); failed != "" {
			t.Fatalf("failed with %q", failed)
		}
	...
	t.Run("non-2xx", func(t *testing.T) {
		...
		_, failed := runCLI(t, func() { startDeviceAuth(server.URL) })
	...
	t.Run("unreachable", func(t *testing.T) {
		url := deadServerURL(t)
		_, failed := runCLI(t, func() { startDeviceAuth(url) })
...
func TestResolveLoginBaseURL(t *testing.T) {
	...
	if got := resolveLoginBaseURL("https://flag.example.com/"); got != "https://flag.example.com" {
		t.Errorf("the flag should win and lose its trailing slash, got %q", got)
	}

	withStdin(t, "https://typed.example.com\n")
	got := ""
	out, failed := runCLI(t, func() { got = resolveLoginBaseURL("") })
	if failed != "" {
		t.Fatalf("failed with %q", failed)
	}
...
func TestResolveLoginBaseURLFallsBackToTheStoredOne(t *testing.T) {
	isolate(t)
	if err := writeStoredConfig(StoredConfig{APIKey: "k", BaseURL: "https://stored.example.com"}); err != nil {
		t.Fatal(err)
	}
	withStdin(t, "\n")
...
func TestResolveLoginBaseURLNeedsAnAnswer(t *testing.T) {
	...
	_, failed := runCLI(t, func() { resolveLoginBaseURL("") })
...
func TestRevokeAPIKey(t *testing.T) {
	t.Run("ok", func(t *testing.T) {
		...
		if !revokeAPIKey(StoredConfig{APIKey: "k", BaseURL: server.URL}) {
	...
	t.Run("unreachable", func(t *testing.T) {
		if revokeAPIKey(StoredConfig{APIKey: "k", BaseURL: deadServerURL(t)}) {
...
func TestLogout(t *testing.T) {
	...
	t.Run("revoked", func(t *testing.T) {
		...
		server := httptest.NewServer(http.HandlerFunc(func(http.ResponseWriter, *http.Request) {}))
		t.Cleanup(server.Close)
		if err := writeStoredConfig(StoredConfig{APIKey: "k", BaseURL: server.URL}); err != nil {
			t.Fatal(err)
		}
	...
	t.Run("clears the config even when the instance is unreachable", func(t *testing.T) {
		isolate(t)
		if err := writeStoredConfig(StoredConfig{APIKey: "k", BaseURL: deadServerURL(t)}); err != nil {
			t.Fatal(err)
		}
...
func TestAssetSuffixMatchesThisPlatform(t *testing.T) {
	...
	if got, failed := "", ""; true {
		_, failed = runCLI(t, func() { got = assetSuffix() })
		if failed != "" {
			t.Fatalf("failed with %q", failed)
		}
		if got != want {
...
func TestLatestReleaseTag(t *testing.T) {
	t.Run("ok", func(t *testing.T) {
		...
		tag := ""
		if _, failed := runCLI(t, func() { tag = latestReleaseTag() }); failed != "" {
			t.Fatalf("failed with %q", failed)
		}
		if tag != "v1.2.3" {
			t.Errorf("want v1.2.3, got %q", tag)
		}
		if seenPath != "/repos/"+githubRepo+"/releases/latest" {
			t.Errorf("wrong path %q", seenPath)
		}
	})
	...
	t.Run("non-2xx", func(t *testing.T) {
		...
		_, failed := runCLI(t, func() { latestReleaseTag() })
...
func stubGitHub(t *testing.T, apiBase, downloadBase string) {
	t.Helper()
	originalAPI, originalDownload := githubAPIBase, githubDownloadBase
	githubAPIBase, githubDownloadBase = apiBase, downloadBase
	t.Cleanup(func() { githubAPIBase, githubDownloadBase = originalAPI, originalDownload })
}
...
func TestDownloadReleaseUnpacksNextToTheBinary(t *testing.T) {
	...
	staged := ""
	if _, failed := runCLI(t, func() { staged = downloadRelease("v1.2.3", assetSuffix(), dest) }); failed != "" {
		t.Fatalf("failed with %q", failed)
	}

	if want := fmt.Sprintf("/%s/releases/download/v1.2.3/homerun-cli-%s.gz", githubRepo, assetSuffix()); seenPath != want {
...
func TestDownloadReleaseFailures(t *testing.T) {
	t.Run("non-2xx", func(t *testing.T) {
		...
		_, failed := runCLI(t, func() { downloadRelease("v1.2.3", assetSuffix(), filepath.Join(t.TempDir(), "homerun")) })
...
func TestRequireClientWithoutALogin(t *testing.T) {
	...
	_, failed := runCLI(t, func() { requireClient("", "") })
	if !strings.Contains(failed, "Not logged in") {
		t.Errorf("got %q", failed)
	}

	if _, failed := runCLI(t, func() { requireClient("https://example.com", "k") }); failed != "" {
...
func TestRunScansDefaultsToList(t *testing.T) {
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			out, failed := runCLI(t, func() { runScans(func() *Client { return client }, test.args) })
			if failed != "" {
				t.Fatalf("failed with %q", failed)
			}
			if (*seen)[0].Path != test.wantPath {
...
func TestRunScansNeedsAnID(t *testing.T) {
	for name, args := range map[string][]string{"list": {"list"}, "get": {"get"}, "bare": {}} {
		_, failed := runCLI(t, func() {
			runScans(func() *Client { return newClient(ClientConfig{BaseURL: "https://example.com"}) }, args)
		})
		if !strings.Contains(failed, "missing <id>") {
			t.Errorf("%s: got %q", name, failed)
		}
...
func TestRunServiceScanValidatesFailOn(t *testing.T) {
	client := func() *Client { return newClient(ClientConfig{BaseURL: "https://example.com"}) }

	_, failed := runCLI(t, func() { runServiceScan(client, []string{"svc-1", "--fail-on", "nope"}) })
...
func TestRunServiceScanImpliesWait(t *testing.T) {
	...
	if _, failed := runCLI(t, func() {
		runServiceScan(func() *Client { return client }, []string{"svc-1", "--fail-on", "critical"})
	}); failed != "" {
		t.Fatalf("failed with %q", failed)
	}
	if len(*seen) < 3 {
...
func TestRunServicesDispatch(t *testing.T) {
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			_, failed := runCLI(t, func() {
				runServices(globalFlags{apiKey: "k", baseURL: server.URL}, test.args)
			})
			if failed != "" {
				t.Fatalf("failed with %q", failed)
			}
...
func TestRunServicesRejectsBadInput(t *testing.T) {
	for name, test := range map[string]struct {
		...
		t.Run(name, func(t *testing.T) {
			isolate(t)
			_, failed := runCLI(t, func() { runServices(globalFlags{}, test.args) })
			if !strings.Contains(failed, test.want) {
				t.Errorf("want %q, got %q", test.want, failed)
			}
		})
...
func TestRunStacksAndTemplates(t *testing.T) {
	for name, run := range map[string]func(globalFlags, []string){
		"stacks":    runStacks,
		...
		t.Run(name, func(t *testing.T) {
			...
			if _, failed := runCLI(t, func() {
				run(globalFlags{apiKey: "k", baseURL: server.URL}, []string{"list", "--per-page", "5"})
			}); failed != "" {
				t.Fatalf("failed with %q", failed)
			}
			...
			for _, args := range [][]string{nil, {"delete"}} {
				_, failed := runCLI(t, func() { run(globalFlags{}, args) })
...
func TestRunInstanceDispatch(t *testing.T) {
	...
	t.Cleanup(server.Close)
	global := globalFlags{apiKey: "k", baseURL: server.URL}

	out, failed := runCLI(t, func() { runInstance(global, []string{"status", "--json"}) })
	...
	out, failed = runCLI(t, func() { runInstance(global, []string{"update"}) })
	...
	for name, test := range map[string]struct {
		...
		if _, failed := runCLI(t, func() { runInstance(global, test.args) }); !strings.Contains(failed, test.want) {
			t.Errorf("%s: got %q", name, failed)
		}
	}
}
...
func TestSplitGlobalFlagsNeedsAValue(t *testing.T) {
	_, failed := runCLI(t, func() { splitGlobalFlags([]string{"services", "--base-url"}) })
...
func TestMainDispatch(t *testing.T) {
	...
	if _, failed := runCLI(t, main); !strings.Contains(failed, `unknown command "frobnicate"`) {
		t.Errorf("got %q", failed)
	}

	os.Args = []string{"homerun", "logout"}
	if out, failed := runCLI(t, main); failed != "" || out != "Not logged in.\n" {
...
func TestLoginEndToEnd(t *testing.T) {
	...
	out, failed := runCLI(t, func() { login(server.URL) })
	if failed != "" {
		t.Fatalf("failed with %q", failed)
	}
	for _, want := range []string{
	...
	if stored := readStoredConfig(); stored == nil || stored.APIKey != "key-1" {
...
func TestSelfUpdateStopsWhenAlreadyCurrent(t *testing.T) {
	...
	out, failed := runCLI(t, selfUpdate)
	if failed != "" {
		t.Fatalf("failed with %q", failed)
	}
...
func TestPrintJSONFallsBackToTheRawBytes(t *testing.T) {
	out := captureStdout(t, func() { printJSON([]byte("not json at all\n")) })
...
func TestPrintValueFailsOnSomethingUnencodable(t *testing.T) {
	_, failed := runCLI(t, func() { printValue(make(chan int)) })
...
func TestConfigPathWithoutAHome(t *testing.T) {
	t.Setenv("HOME", "")
	if got := configPath(); got != filepath.Join(".config", "homerun", "config.json") {
...
func TestResolveConfigFillsTheGapsFromTheStoredLogin(t *testing.T) {
	isolate(t)
	if err := writeStoredConfig(StoredConfig{APIKey: "stored-key", BaseURL: "https://stored.example.com"}); err != nil {
		t.Fatal(err)
	}

	config := resolveConfig("https://flag.example.com", "")
	...
	t.Setenv("HOME", t.TempDir())
	if config := resolveConfig("https://flag.example.com", ""); config != nil {
		t.Errorf("half a config is not a login, got %+v", config)
	}
}

func TestSplitGlobalFlagsAcceptsAnExplicitEmptyValue(t *testing.T) {
	global, rest := splitGlobalFlags([]string{"--api-key=", "services", "list"})
	...
	global, rest = splitGlobalFlags([]string{"--base-url=", "services"})
...
func TestShortenCutsOnRuneBoundaries(t *testing.T) {
	if got := shorten("héllo wörld", 7); got != "héllo w" {
		t.Errorf("a multi-byte character must not be cut in half, got %q", got)
	}
	if got := shorten("abc", 7); got != "abc" {
		t.Errorf("a short value is returned whole, got %q", got)
	}
}

func TestRequirePositiveTimeout(t *testing.T) {
	if _, failed := runCLI(t, func() { requirePositiveTimeout(-5) }); !strings.Contains(failed, "can't be negative") {
		t.Errorf("a negative timeout should be rejected, got %q", failed)
	}
	if _, failed := runCLI(t, func() { requirePositiveTimeout(0) }); failed != "" {
...
func TestInstanceUpdateRefusesAVersionlessAnswer(t *testing.T) {
	...
	_, failed := runCLI(t, func() { instanceUpdate(client, true, 0) })
