...
```bash
go run ./packages/cli services list             # from source
go test ./packages/cli/...                      # its unit tests (bun run test:unit:cli)
go vet ./packages/cli/...                       # bun run check:cli
gofmt -w ./packages/cli                         # CI fails on anything gofmt would rewrite
