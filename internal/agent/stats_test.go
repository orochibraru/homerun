...
func TestSampleReadsProcDfAndNvidiaSmi(t *testing.T) {
	...
	sampler := &StatsSampler{procRoot: root, runCommand: commands(map[string]string{
...
func TestSampleRejectsMalformedSources(t *testing.T) {
	...
	if (&StatsSampler{procRoot: root, runCommand: commands(nil)}).Sample().CPUPercent != 0 {
	...
	zeroDisk := &StatsSampler{procRoot: t.TempDir(), runCommand: commands(map[string]string{
		"df": "h\n/dev/x 0 0 0 0% /\n",
	})}
...
func TestNewStatsSamplerUsesTheRealHost(t *testing.T) {
	...
	if _, ok := runCommand("definitely-not-a-command-homerun"); ok {
