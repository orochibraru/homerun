...
func TestResolveTokenExplicitWins(t *testing.T) {
	...
	token, source, err := resolveToken("from-env", file)
	if err != nil || token != "from-env" || source != TokenFromEnv {
...
func TestResolveTokenGeneratesPersistsAndReuses(t *testing.T) {
	...
	first, source, err := resolveToken("", file)
	if err != nil || source != TokenGenerated || len(first) != 64 {
	...
	second, source, err := resolveToken("", file)
	if err != nil || source != TokenPersisted || second != first {
...
func TestResolveTokenRegeneratesAWhitespaceFile(t *testing.T) {
	...
	token, source, err := resolveToken("", file)
	if err != nil || source != TokenGenerated || token == "" {
...
func TestResolveTokenTrimsAPersistedToken(t *testing.T) {
	...
	token, source, err := resolveToken("", file)
	if err != nil || token != "abc123" || source != TokenPersisted {
...
func TestResolveTokenReportsAnUnwritableLocation(t *testing.T) {
	...
	if _, _, err := resolveToken("", filepath.Join(blocker, "token")); err == nil {
...
func TestTokensMatch(t *testing.T) {
	...
	for _, testCase := range cases {
		if got := tokensMatch(testCase.a, testCase.b); got != testCase.want {
