...
//
// Only the worker and Traefik see the socket. The app makes no Docker calls of
// its own, it asks the worker's control API over WORKER_URL, and both sides
// derive the bearer token from the shared AUTH_SECRET when WORKER_TOKEN is
// unset, which is why nothing here has to generate one.
//
...
services:
  ...
  worker:
    ...
    environment:
      DATABASE_URL: postgres://${POSTGRES_USER:-homerun}:${POSTGRES_PASSWORD:-homerun}@postgres:5432/${POSTGRES_DB:-homerun}
      AUTH_SECRET: "${AUTH_SECRET:?missing from .env - the installer generates this automatically}"
...
volumes:
  ...
  homerun-data: {}
`,
		image, host, host, dashboardResolver,
		image, dockerSocket, dockerSocket, dockerSocket,
