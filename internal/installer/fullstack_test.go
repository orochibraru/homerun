...
func TestFullStackComposeShape(t *testing.T) {
	compose := fullStackCompose("docker.io/orochibraru/homerun:v1.2.3", "/var/run/docker.sock", "homerun.example.com", false)
...
func TestFullStackComposeRunsTheWorkerFromTheAppImage(t *testing.T) {
	compose := fullStackCompose("docker.io/orochibraru/homerun:1.2.3", "/run/user/1000/docker.sock", "example.com", false)
...
func TestFullStackComposeBareIPHasNoCertResolver(t *testing.T) {
	compose := fullStackCompose("img", "/var/run/docker.sock", "203.0.113.10", false)
...
func TestFullStackComposeSwarm(t *testing.T) {
	compose := fullStackCompose("img", "/var/run/docker.sock", "homerun.example.com", true)
	for _, fragment := range []string{
...
func TestFullStackComposeMountsTraefikSocketAtTheConventionalPath(t *testing.T) {
	compose := fullStackCompose("img", "/run/user/1000/docker.sock", "homerun.example.com", false)
...
func TestEnsureAuthSecretGeneratesOnceAndTightensPermissions(t *testing.T) {
	...
	if err := EnsureAuthSecret(run, "homerun", composeDir); err != nil {
		t.Fatal(err)
	}
...
func TestEnsureConfigFileNeverClobbers(t *testing.T) {
	...
	if err := ensureConfigFile(run, configPath, "/var/run/docker.sock", "homerun.example.com"); err != nil {
		t.Fatal(err)
	}
	...
	run = newFakeRunner()
	if err := ensureConfigFile(run, configPath, "/var/run/docker.sock", "other.example.com"); err != nil {
		t.Fatal(err)
	}
	if len(run.writes) != 0 {
...
func TestBringUpFullStackRootless(t *testing.T) {
	...
	original := sysctlConfPath
	sysctlConfPath = sysctl
	t.Cleanup(func() { sysctlConfPath = original })

	run := newFakeRunner()
	composePath, err := BringUpFullStack(FullStackParams{
