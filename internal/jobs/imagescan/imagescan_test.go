...
func TestSummarizeCountsEverySeverityAcrossResults(t *testing.T) {
	summary, err := Summarize(report(t,
	...
	want := Counts{Critical: 1, High: 1, Low: 1, Medium: 1, Unknown: 1}
...
func TestSummarizeFixableAndDedupe(t *testing.T) {
	summary, err := Summarize(report(t, vulns(
	...
	if summary.Counts != (Counts{Critical: 2, High: 1}) || summary.FixableCounts != (Counts{Critical: 1}) || summary.TotalFindings != 3 {
		t.Fatalf("got %+v", summary)
	}
}
...
func TestSummarizeFieldsMatchTheApp(t *testing.T) {
	summary, err := Summarize(report(t, vulns(vulnerability("CVE-9", "critical", "openssl", true))), MaxStoredFindings)
	if err != nil {
		t.Fatal(err)
	}
...
func TestSummarizeCleanAndInvalid(t *testing.T) {
	summary, err := Summarize([]byte(`{"SchemaVersion":2}`), MaxStoredFindings)
	...
	for _, raw := range []string{"FATAL no such image", "[]", "null"} {
		if _, err := Summarize([]byte(raw), MaxStoredFindings); err == nil || err.Error() != "Trivy didn't return a JSON report." {
...
func TestLastErrorLine(t *testing.T) {
	...
	for input, want := range cases {
		if got := LastErrorLine(input); got != want {
...
func TestCommand(t *testing.T) {
	remote := Command("homerun-mirror:5000/x:1", Source{Insecure: true, Kind: "remote"})
	if !reflect.DeepEqual(remote[:4], []string{"image", "--image-src", "remote", "--insecure"}) || remote[len(remote)-1] != "homerun-mirror:5000/x:1" {
		t.Fatalf("remote: %v", remote)
	}
	if got := Command("app:1", Source{Kind: "docker"})[2]; got != "docker" {
		t.Fatal(got)
	}
	if got := Command("app:1", Source{Kind: "any"})[2]; got != "docker,remote" {
		t.Fatal(got)
	}
	if strings.Contains(strings.Join(Command("r/app:1", Source{Kind: "remote"}), " "), "--insecure") {
...
func (d *fakeDaemon) serve(t *testing.T) *dockerapi.Client {
	...
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		defer d.mu.Unlock()
		switch {
		case r.URL.Path == "/images/"+trivyImage+"/json":
			_, _ = w.Write([]byte(`{}`))
...
func TestScanRunsTrivyAndSummarizes(t *testing.T) {
	...
	client := d.serve(t)
	summary, err := Scan(context.Background(), client, "/var/run/docker.sock", "homerun", Target{
		Auth:   &dockerapi.AuthConfig{Password: "pw", Username: "me"},
		Ref:    "nginx:alpine",
		Source: Source{Kind: "any"},
	})
	if err != nil {
		t.Fatal(err)
	}
...
func TestScanReportsTheScannerError(t *testing.T) {
	...
	_, err := Scan(context.Background(), d.serve(t), "/sock", "", Target{Ref: "gone:1", Source: Source{Kind: "remote"}})
