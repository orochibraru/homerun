...
func stubContextHost(t *testing.T, host string) {
	t.Helper()
	original := contextHost
	contextHost = func() string { return host }
	t.Cleanup(func() { contextHost = original })
}

func TestResolvePrefersExplicit(t *testing.T) {
	if got := Resolve("/explicit.sock"); got != "/explicit.sock" {
		t.Errorf("got %q", got)
	}
}
...
func TestDetect(t *testing.T) {
	...
	if got := Detect(); got != "/from/docker-host.sock" {
		t.Errorf("DOCKER_HOST wins, got %q", got)
	}

	t.Setenv("DOCKER_HOST", "tcp://remote:2375")
	if got := Detect(); got != "/from/context.sock" {
	...
	if got := Detect(); got != "/var/run/docker.sock" {
