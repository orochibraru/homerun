...
func (f *fakeS3) handler(t *testing.T) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		want, _ := Signature("secret", "us-east-1", headers["x-amz-date"], r.Method, r.URL.EscapedPath(),
			canonicalQuery(r.URL.Query()), headers, sha256Hex(body))
...
func TestUploadAbortsOnFailedPart(t *testing.T) {
	client, fake := newFake(t)
	fake.failAt = 2
	if _, err := client.Upload(context.Background(), "big.tar.gz", bytes.NewReader(make([]byte, PartSize+1))); err == nil {
