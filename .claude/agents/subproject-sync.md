...
`packages/installer/`, `packages/cli/`. Each has its own `package.json`...
actually no — they share the root `package.json`/`bun install`, but each has its
**own `tsconfig.json`** and is typechecked separately
(`check:agent`/`check:installer`/`check:cli` root scripts, or
`bunx tsc --noEmit -p <dir>/tsconfig.json` directly). None of them are covered
by `bun run check` (that's `svelte-kit sync && svelte-check`, SvelteKit-only).
...
from a **running instance's real** `/api/v1/openapi.json` — it is checked in as
a snapshot and goes stale silently (no compile-time staleness detection) if the
...
`bun run generate` inside `packages/cli/` (check `packages/cli/package.json` for
the exact script name). Flag to the user explicitly if you can't actually run
this (e.g. no live instance available) rather than leaving the snapshot silently
stale — this is a documented sharp edge in this repo, not a hypothetical.
