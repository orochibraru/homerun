...
// newFakeRunner builds an empty fakeRunner with no failures or answers set.
...
// Run implements installer.Runner.
func (f *fakeRunner) Run(cmd []string, opts installer.Opts) (installer.Result, error) {
	f.calls = append(f.calls, fakeCall{Cmd: cmd, Opts: opts})
	joined := strings.Join(cmd, " ")
	for substr := range f.failures {
		if strings.Contains(joined, substr) {
			return installer.Result{Code: 1}, &commandError{command: joined}
		}
	}
	for substr, out := range f.stdout {
		if strings.Contains(joined, substr) {
			return installer.Result{Stdout: out}, nil
		}
	}
	return installer.Result{}, nil
}

// RunOK implements installer.Runner.
func (f *fakeRunner) RunOK(cmd []string, opts installer.Opts) bool {
	_, err := f.Run(cmd, opts)
	return err == nil
}

// WriteFile implements installer.Runner.
func (f *fakeRunner) WriteFile(path, content string) error {
	f.writes[path] = content
	return nil
}

// AppendLine implements installer.Runner.
...
// Error returns the failed command's argv.
...
func withHomeRoot(t *testing.T, username string) string {
	t.Helper()
	root := t.TempDir()
	original := installer.HomeRoot
	installer.HomeRoot = root
	t.Cleanup(func() { installer.HomeRoot = original })
...
// stubCommandExists makes installer.CommandExists answer from a fixed set for one test.
func stubCommandExists(t *testing.T, present ...string) {
	t.Helper()
	original := installer.CommandExists
	installer.CommandExists = func(cmd string) bool {
	...
	t.Cleanup(func() { installer.CommandExists = original })
}

// stubCommandOutput makes installer.CommandOutput answer from a map keyed by the joined
// command, for one test.
func stubCommandOutput(t *testing.T, answers map[string]string) {
	t.Helper()
	original := installer.CommandOutput
	installer.CommandOutput = func(cmd []string) string { return answers[strings.Join(cmd, " ")] }
	t.Cleanup(func() { installer.CommandOutput = original })
}
