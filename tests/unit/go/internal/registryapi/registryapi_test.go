...
func TestPlanGC(t *testing.T) {
	inventory := []registryapi.Tag{
		...
		{Digest: digest("9"), Repository: alpine, Tag: registryapi.KeepTagFor(digest("9"))},
		{Digest: digest("5"), Repository: nginx, Tag: "1.27"},
	}
	keep := registryapi.KeepSet{
		Digests: []registryapi.Digest{
			{Digest: digest("2"), Repository: alpine},
			{Digest: digest("7"), Repository: alpine},
			{Digest: digest("8"), Repository: "docker.io/library/gone"},
		},
		Tags: []registryapi.TagRef{{Repository: alpine, Tag: "3.20"}},
	}
	plan := registryapi.PlanGC(inventory, []string{alpine, nginx, "ghcr.io/empty"}, keep)

	wantDeletes := []registryapi.Digest{{Digest: digest("3"), Repository: alpine}, {Digest: digest("9"), Repository: alpine}, {Digest: digest("5"), Repository: nginx}}
	if !reflect.DeepEqual(plan.Deletes, wantDeletes) {
		t.Errorf("deletes = %v, want %v", plan.Deletes, wantDeletes)
	}
	wantPins := []registryapi.Tag{{Digest: digest("7"), Repository: alpine, Tag: registryapi.KeepTagFor(digest("7"))}}
...
func TestPlanGCEmptyKeepDeletesEverything(t *testing.T) {
	plan := registryapi.PlanGC([]registryapi.Tag{{Digest: digest("1"), Repository: alpine, Tag: "3.20"}}, []string{alpine}, registryapi.KeepSet{})
...
func TestHelpers(t *testing.T) {
	if got := registryapi.NextCatalogPath(`</v2/_catalog?last=b&n=1000>; rel="next"`); got != "/v2/_catalog?last=b&n=1000" {
		t.Errorf("next = %q", got)
	}
	if registryapi.NextCatalogPath("") != "" {
		t.Error("no link should have no next page")
	}
	if !registryapi.IsValidRepository(alpine) || registryapi.IsValidRepository("../etc") || registryapi.IsValidRepository("a/../../b") {
...
// fakeRegistry starts a fake registry serving routes and returns a client
// pointed at it, plus the calls it recorded.
func fakeRegistry(t *testing.T, routes map[string]http.HandlerFunc) (*registryapi.Client, *[]recorded) {
	t.Helper()
	var calls []recorded
	...
	return registryapi.New(server.URL+"/", "", ""), &calls
}
...
func TestInventoryResolvesDigestsWithIndexAwareAccept(t *testing.T) {
	...
	if err != nil || !reflect.DeepEqual(got, []registryapi.Tag{{Digest: digest("1"), Repository: alpine, Tag: "3.20"}}) {
		t.Fatalf("inventory = %v, %v", got, err)
	}
	if (*calls)[1].accept != registryapi.ManifestAccept || !strings.Contains(registryapi.ManifestAccept, "application/vnd.oci.image.index.v1+json") {
...
func TestDeleteManifest(t *testing.T) {
	...
	if gone, err := client.DeleteManifest(ctx, registryapi.Digest{Digest: digest("1"), Repository: alpine}); !gone || err != nil {
		t.Errorf("delete = %v, %v", gone, err)
	}
	if gone, err := client.DeleteManifest(ctx, registryapi.Digest{Digest: digest("2"), Repository: alpine}); gone || err != nil {
		t.Errorf("already gone = %v, %v", gone, err)
	}
	if _, err := client.DeleteManifest(ctx, registryapi.Digest{Digest: digest("3"), Repository: alpine}); err == nil || !strings.Contains(err.Error(), "HTTP 405") {
...
func TestTagManifestRePutsTheManifest(t *testing.T) {
	...
	client, calls := fakeRegistry(t, map[string]http.HandlerFunc{
		...
		"PUT /v2/" + alpine + "/manifests/" + registryapi.KeepTagFor(digest("7")): func(w http.ResponseWriter, _ *http.Request) {
			w.WriteHeader(http.StatusCreated)
		},
	})
	ctx := context.Background()
	pinned, err := client.TagManifest(ctx, registryapi.Tag{Digest: digest("7"), Repository: alpine, Tag: registryapi.KeepTagFor(digest("7"))})
	if !pinned || err != nil {
	...
	if pinned, err := client.TagManifest(ctx, registryapi.Tag{Digest: digest("8"), Repository: alpine, Tag: registryapi.KeepTagFor(digest("8"))}); pinned || err != nil {
		t.Errorf("missing source = %v, %v", pinned, err)
	}
}

func TestPingAndBasicAuth(t *testing.T) {
	if registryapi.New("http://127.0.0.1:1", "", "").Ping(context.Background()) {
	...
	defer server.Close()
	if !registryapi.New(server.URL, "homerun-internal", "secret").Ping(context.Background()) || user != "homerun-internal" {
