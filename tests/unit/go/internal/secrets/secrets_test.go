...
// loadFixture reads the Go/TypeScript encryption parity fixture.
...
// mustBox builds a secrets.Box for secret, failing the test on error.
func mustBox(t *testing.T, secret string) *secrets.Box {
	t.Helper()
...
func TestAuthSecretFromEnv(t *testing.T) {
	...
	if got := secrets.AuthSecretFromEnv(); got != "from-better-auth" {
		t.Errorf("got %q", got)
	}
	t.Setenv("BETTER_AUTH_SECRET", "")
	if got := secrets.AuthSecretFromEnv(); got != "default-secret" {
		t.Errorf("got %q", got)
	}
	t.Setenv("AUTH_SECRET", "primary")
	if got := secrets.AuthSecretFromEnv(); got != "primary" {
		t.Errorf("got %q", got)
	}
}
