...
func TestAuthenticatedCloneURL(t *testing.T) {
	credential := &agent.GitCredential{Token: "t0k/en", Username: "bot"}
	cases := []struct {
		name       string
		url        string
		credential *agent.GitCredential
		want       string
	}{
	...
	for _, testCase := range cases {
		if got := agent.AuthenticatedCloneURL(testCase.url, testCase.credential); got != testCase.want {
...
func TestRedactCloneURLKeepsSecretsOutOfLogs(t *testing.T) {
	if got := agent.RedactCloneURL("https://bot:SECRET@github.com/o/r.git"); strings.Contains(got, "SECRET") {
		t.Errorf("a bare URL must be redacted, got %q", got)
	}
	message := "fatal: unable to access 'https://bot:SECRET@github.com/o/r.git/': denied"
	got := agent.RedactCloneURL(message)
	...
	if got := agent.RedactCloneURL("nothing to see"); got != "nothing to see" {
		t.Errorf("plain text should pass through, got %q", got)
	}
	if got := agent.RedactCloneURL(""); got != "" {
		t.Errorf("got %q", got)
	}
}

func TestGitCheckoutSteps(t *testing.T) {
	branch := agent.GitCheckoutSteps("https://x/r", "main", "/w/r")
	...
	steps := agent.GitCheckoutSteps("https://x/r", sha, "/w/r")
	if len(steps) != 4 {
...
func TestIsCommitSHAAndExtract(t *testing.T) {
	...
	if !agent.IsCommitSHA(sha) || !agent.IsCommitSHA(" "+strings.ToUpper(sha)+" ") {
		t.Error("a full SHA in either case is a commit")
	}
	if agent.IsCommitSHA("main") || agent.IsCommitSHA(sha[:39]) {
		t.Error("a branch or a short SHA is not")
	}
	if got := agent.ExtractCommitSHA("\x01\x00\x00\x00\x00\x00\x00\x29" + sha + "\n"); got != sha {
		t.Errorf("should read through a Docker frame header, got %q", got)
	}
	if got := agent.ExtractCommitSHA("no commit here"); got != "" {
		t.Errorf("got %q", got)
	}
}
