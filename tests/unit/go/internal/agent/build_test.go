...
func gitSucceeds(config dockerapi.ContainerConfig) (int, string) {
	if config.Image == agent.GitImage && strings.Contains(strings.Join(config.Cmd, " "), "rev-parse") {
		return 0, headSHA + "\n"
	}
	return 0, "#1 building\n#2 done\n"
}

// newTestBuilder builds an agent.Builder over docker with a fixed NewID, for
// deterministic assertions on generated names.
func newTestBuilder(docker *fakeDocker) *agent.Builder {
	builder := agent.NewBuilder(docker, "/var/run/docker.sock")
...
func TestBuildHappyPathCleansUpAfterItself(t *testing.T) {
	...
	result := newTestBuilder(docker).Build(context.Background(), agent.BuildInput{
	...
	for _, image := range []string{agent.GitImage, agent.Tools.HelperImage} {
		if !docker.called("pull " + image) {
			t.Errorf("%s isn't present yet, so it should be pulled", image)
		}
	}
	if !docker.called("create " + agent.GitImage + " clone --depth 1 --branch main") {
...
func TestBuildRunsTheBuilderWithTheSocketAndTools(t *testing.T) {
	docker := newFakeDocker()
	docker.images[agent.GitImage] = true
	docker.images[agent.Tools.HelperImage] = true
	docker.run = gitSucceeds
	...
	for _, config := range docker.created {
		if config.Image == agent.Tools.HelperImage {
			builder = config
		}
	}
	if builder.Cmd[1] != agent.BuilderScript {
		t.Error("the builder runs the embedded script")
	}
	wantBinds := []string{"homerun-agent-build-abcd1234:/workspace", agent.Tools.ToolsVolume + ":/tools", "/var/run/docker.sock:/var/run/docker.sock"}
...
// contains reports whether want is one of values.
...
func TestBuildPinsAMovedBranchToTheCheckedCommit(t *testing.T) {
	...
	if !docker.called("create " + agent.GitImage + " -C /workspace/repo checkout --detach " + pinned) {
...
func TestBuildFailuresAreReported(t *testing.T) {
	cases := []struct {
	...
	}{
		{
			...
			input:   agent.BuildInput{GitRef: ptr("nope"), GitURL: "https://github.com/o/r.git", Tag: "svc:5"},
		...
		},
		{
			...
			input:   agent.BuildInput{GitURL: "https://github.com/o/r.git", Tag: "svc:6"},
		...
		},
		{
			...
			input:   agent.BuildInput{GitURL: "https://github.com/o/r.git", Tag: "svc:7"},
			wantErr: "boom",
		},
		{
			name:    "an invalid build context",
			setup:   func(docker *fakeDocker) { docker.run = gitSucceeds },
			input:   agent.BuildInput{BuildContext: ptr("../../etc"), GitURL: "https://github.com/o/r.git", Tag: "svc:8"},
		...
		},
		{
			...
			input: agent.BuildInput{
				GitURL: "https://github.com/o/r.git",
				Push:   &agent.PushTarget{RegistryURL: "r.example.com", Tag: "r.example.com/svc:9"},
				Tag:    "svc:9",
			},
			wantErr: "boom",
			wantSHA: true,
		},
		{
			...
			input:   agent.BuildInput{GitURL: "https://github.com/o/r.git", Tag: "svc:10"},
			wantErr: "boom",
		},
	}
...
func TestBuildRedactsTheCredentialFromItsError(t *testing.T) {
	...
	result := newTestBuilder(docker).Build(context.Background(), agent.BuildInput{
		Credential: &agent.GitCredential{Token: "SECRET", Username: "bot"},
		GitURL:     "https://github.com/o/r.git",
		Tag:        "svc:11",
	})
...
func TestBuildHonoursAClosedRequest(t *testing.T) {
	...
	cancel()
	result := newTestBuilder(docker).Build(ctx, agent.BuildInput{GitURL: "https://github.com/o/r.git", Tag: "svc:12"})
	if result.Success {
...
func TestScanLinesAnyEnding(t *testing.T) {
	...
	for _, testCase := range cases {
		advance, token, _ := agent.ScanLinesAnyEnding([]byte(testCase.data), testCase.atEOF)
