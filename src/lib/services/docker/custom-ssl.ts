...
/**
 * Writes (or removes) a service's custom SSL cert/key plus a Traefik
 * dynamic-config file into `config.traefik.dynamicConfigDir`. Doesn't
 * need a Docker client at all (this is filesystem-only), but is still a
 * concern class merged into DockerService like the others, rather than a
 * bag of loose functions.
 */
export function DockerCustomSslMixin<
	TBase extends Constructor<BaseDockerService>,
>(Base: TBase) {
	return class DockerCustomSslService extends Base {
		/**
		 ...
		 */
		async syncCustomSslConfig(svc: CustomSslService): Promise<void> {
			...
			if (!hasCert) {
				...
				// files rather than leaving a stale cert Traefik might still
				// be serving for a domain that's since changed.
				await Promise.all([
			...
			const cert = svc.customSslCertEnc
				...
				: null;
			const key = svc.customSslKeyEnc
				? decryptSecret(svc.customSslKeyEnc)
				: null;
			if (!(cert && key)) {
				logger.warn(
					`Couldn't decrypt custom SSL cert/key: service=${svc.slug}`,
				);
				return;
			}
