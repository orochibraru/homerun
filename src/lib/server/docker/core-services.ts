 ...
 * fallback too).
 */
export async function findTraefikContainer(): Promise<TraefikInfo | null> {
 ...
 */
export async function restartTraefikContainer(): Promise<void> {
	const traefik = await findTraefikContainer();
	if (!traefik) {
		throw new Error("Traefik container not found.");
	}
	await getDocker().getContainer(traefik.id).restart();
...
export async function updateTraefikContainer(): Promise<TraefikUpdateResult> {
	...
	if (pulled.Id === info.Image) {
		...
		return { message: `Already running the latest ${ref}.`, updated: false };
	}
	...
	const endpointsConfig = Object.fromEntries(
		Object.entries(info.NetworkSettings.Networks ?? {}).map(([netName, ep]) => [
			netName,
