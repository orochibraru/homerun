<script lang="ts">
	...
	const view = new ViewMode("project-services");
	let search = $state("");
...
<div class="p-5 md:p-6">
  ...
  <div class="border-border mb-5 flex flex-wrap items-end justify-between gap-3 border-b pb-4">
    <div>
      <h1 class="text-text text-lg font-semibold tracking-tight">{proj.name}</h1>
      <p class="text-text-muted mt-0.5 text-xs">
        {proj.description ?? `Services on the ${proj.slug} network.`}
      </p>
    </div>
    <div class="flex flex-wrap gap-2">
      <Button
        href={resolve("/(protected)/projects/[projectId]/settings", {
          projectId: proj.id,
        })}
        size="sm"
        variant="outline"
      ...
      <Button href="{resolve('/services/new')}?projectId={proj.id}" size="sm">
        <Plus class="size-4" />
        Add Service
      </Button>
    </div>
    ...
    <div class="mb-4 panel rounded-xl">
      ...
      {#if data.recentDeployments.length === 0}
        <p class="text-text-muted px-4 py-6 text-center text-xs">
          Nothing deployed in this project yet.
        </p>
      {:else}
