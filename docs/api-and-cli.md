...
Every release publishes a compiled `homerun-cli-<arch>` binary (linux/amd64 + linux/arm64) — that's the normal way to get it, no Bun/build step needed:
...
Or, working on the CLI itself, from the repo root: `bun install && bun run cli/index.ts services list` (`bun run scripts/build-packages.ts <amd64|arm64>` compiles it the same way CI does). See [`cli/README.md`](../cli/README.md).
