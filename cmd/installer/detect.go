...
// Arch is this host's release-asset architecture, amd64 or arm64. The
// installer only ever runs on the Linux box it's installing, so it needs no
// darwin- prefix (see internal/release's AssetSuffix, which the CLI uses).
func Arch() (string, error) {
	return release.Arch(runtime.GOARCH)
}
