...
func TestPromptHostReadsStdin(t *testing.T) {
	...
	for _, testCase := range cases {
		...
		var got string
		out := captureStdout(t, func() { got = promptHost(testCase.detected) })
		os.Stdin = original
		read.Close()
...
func TestResolveHostPromptsOnATTY(t *testing.T) {
	...
	var host string
	captureStdout(t, func() { host, err = resolveHost(Options{}) })
	if err != nil {
		t.Fatal(err)
	}
...
func TestResolveHostSkipsThePromptWithYes(t *testing.T) {
	...
	var host string
	var err error
	captureStdout(t, func() { host, err = resolveHost(Options{Yes: true}) })
...
func TestInstallDryRunWholeFlow(t *testing.T) {
	...
	agent := captureStdout(t, func() {
		if err := install(Options{
			AgentPort: 7420, DryRun: true, Mode: ModeAgent, RootlessUser: "homerun", Version: "latest",
		}); err != nil {
			t.Error(err)
		}
	})
	...
	full := captureStdout(t, func() {
		if err := install(Options{
			DryRun: true, Mode: ModeFull, RootlessUser: "homerun", Version: "v1.2.3",
...
func TestInstallSurfacesAStepFailure(t *testing.T) {
	...
	err := install(Options{DryRun: true, Mode: ModeAgent, RootlessUser: "homerun", Version: "latest"})
	if err == nil {
...
func TestMigrateToRootfulPrintsWhatIsLeftByHand(t *testing.T) {
	...
	out := captureStdout(t, func() {
		if err := migrateToRootful(Options{
...
func TestMigrateToRootfulSurfacesAFailure(t *testing.T) {
	...
	err := migrateToRootful(Options{RootlessUser: "homerun"}, run)
...
func TestCopyPending(t *testing.T) {
	...
	if pending, _ := copyPending(volumesFile, stateDir); !pending {
	...
	if _, err := copyPending(volumesFile, stateDir); err == nil {
...
func TestRecordVolumes(t *testing.T) {
	...
	volumes, err := recordVolumes(run, true, Opts{}, volumesFile)
	...
	run = newFakeRunner()
	volumes, err = recordVolumes(run, false, Opts{}, volumesFile)
	...
	if _, err := recordVolumes(newFakeRunner(), true, Opts{}, volumesFile); err == nil {
...
func TestStepFailuresPropagate(t *testing.T) {
	cases := []struct {
		...
		{"engine install", []string{"get.docker.com"}, func(r Runner) error { stubCommandExists(t); return InstallDockerEngine(r) }},
		{"prereqs", []string{"apt-get"}, func(r Runner) error {
			return InstallRootlessPrereqs(r, PackageManager{Install: []string{"apt-get", "install", "-y"}, Kind: "apt"})
		}},
		{"useradd", []string{"id homerun", "useradd"}, func(r Runner) error { return EnsureRootlessUser(r, "homerun") }},
		{"linger", []string{"loginctl"}, func(r Runner) error { _, err := InstallRootlessDocker(r, "homerun"); return err }},
		{"rootful enable", []string{"systemctl enable"}, func(r Runner) error { _, err := EnableRootfulDocker(r); return err }},
		{"docker group", []string{"usermod"}, func(r Runner) error { return AddUserToDockerGroup(r, "homerun") }},
		{"network create", []string{"network"}, func(r Runner) error {
			return EnsureHomerunNetwork(r, "", SystemDockerSocket)
		}},
		{"swarm info", []string{"docker info"}, func(r Runner) error { return EnsureSwarmManager(r, "") }},
		{"agent unit", []string{"mkdir -p"}, func(r Runner) error {
			return InstallAgentSystemdUnit(r, "homerun", "/run/user/1000/docker.sock", 7420)
		}},
		{"compose up", []string{"up -d"}, func(r Runner) error {
			_, err := BringUpFullStack(FullStackParams{
				DockerSocket: SystemDockerSocket, Host: "x", Rootful: true, Run: r, Username: "homerun", Version: "latest",
			})
			return err
		}},
	}
