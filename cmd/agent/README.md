...
them need Go or a source checkout on the target host.
...
(`-arm64` instead of `-amd64` on an arm64 host. No macOS build: like the
installer, the agent only ever runs on the Linux host it's managing, so
`scripts/build-packages.ts` only cross-compiles it for `linux/amd64` and
`linux/arm64`, same as the installer, even though the agent's Go binary could
cross-compile for macOS too.)
...
```bash
go run ./cmd/agent           # from the repo root; also `bun run dev:agent`
```
...
| `DOCKER_SOCKET_PATH`     | _(detected)_             | Point this at a rootless Docker socket (e.g. `/run/user/<uid>/docker.sock`) when installed via `cmd/installer/`'s rootless setup. Unset, it uses a `unix://` `DOCKER_HOST`, then the `docker` CLI's current context, then the first common socket path that exists, falling back to `/var/run/docker.sock`. |
...
- `GET /v1/stats`, host CPU/RAM/disk/GPU, same shape as the main app's
  `SystemStatsService`.
- `POST /v1/build`, body is a `BuildInput` (see `build.go`): clones a git repo
  ...
  helper container with the agent's Docker socket mounted, see `builders.go`.
...
standalone with the same behavior as `go run ./cmd/agent`.

**Also now verified**, against a real disposable Multipass Ubuntu 24.04 VM
provisioned by `cmd/installer/bootstrap.sh --mode=agent` (see
`cmd/installer/README.md` for that run's own findings): running under rootless
Docker specifically, long-running under a real `systemd --user` unit, reachable
over the network from outside the VM (health endpoint + OpenAPI doc both
responded correctly).
